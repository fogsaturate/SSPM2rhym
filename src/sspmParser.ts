...
export const SSPMParser = new Parser() // binary time yay!
// .skip(4) // magic number
.string('magicNumber', {
    length: 4,
    encoding: 'utf8'
...
.array('customDataObject',{
    ...
    type: new Parser()
        ...
        .choice('customData', {
            tag: 'customDataType',
            choices: {
                11: dataTypes.longUTF8Type // this is what grabs the difficulty name! (the difficulty name is a long utf-8 string)
            }
...
.array('markerObject', {
    length: function(this: any) {return this.markerCount;},
    // length: 50,
    type: new Parser()
        ...
        .array('markerData', {
            length: function(this: any) {return this.markerDefinitionIndex + 1}, // adding 1 because length needs to be 1 or else it wont work (it doesnt work off of index logic)
            // length: 1,
            type: new Parser()
                .choice('markerType', {
                    // tag: function(this: any) {return this.markerDefinitions[0].markerDefinitionDataType}, // no matter what i do, this doesnt work
                    // tag: 7,
                    tag: function(this: any) {return 7}, // i cant use integers, nor can i grab from arrays so im just gonna return 7
                    choices: {
                        7: dataTypes.noteType
                    }
                })
        })
});